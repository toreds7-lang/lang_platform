...
# Below this a line is back-channel ("Why?", "Yeah, right.") - understandable
# on sight and worth nothing as shadowing practice.
MIN_WORDS = 5
...
_TAGS = ("key idea", "collocation", "phrasal verb", "discourse marker", "natural rhythm", "idiom")

_RUBRIC = (
    "You are choosing which sentences of a video transcript an English learner should "
    "shadow (listen, then repeat aloud, copying the rhythm).\n\n"
    ...
    "phrasal verbs, discourse markers, contractions and reductions all count.\n\n"
    "Score bands:\n"
    ...
    f"{', '.join(_TAGS)}. Prefer the most specific one: reach for \"key idea\" only when no "
    "language tag describes what makes the sentence worth practising.\n"
...
def _score_batch(batch: list[dict], context_before: list[str], context_after: list[str]):
    ...
    for entry in entries:
        ...
        out[index] = {
            ...
            "tag": _clean_tag(entry.get("tag", "")),
            "reason": _clean_reason(entry.get("reason", "")),
        }
    return out, None


def score_sentences(sentences: list[dict]) -> list[dict]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for batch in batches:
            ...
            futures.append(pool.submit(_score_batch, batch, before, after))
